...
# Global arrays with zero-based indexing where needed
il = OffsetArray(zeros(Int, nx + 1), 0:nx)
ir = OffsetArray(zeros(Int, nx + 1), 0:nx)
coef = OffsetArray(zeros(iorder + 1, iorder), 0:iorder, 0:iorder-1)  # Adjusted to 0-based
dd = OffsetArray(zeros(ighost, ntcell + 1), 0:ighost-1, 0:ntcell)    # Corrected to 0-based
...
function reconstruction(q)
    ...
    dd[0, 1:ntcell] .= q[1:ntcell]  # Corrected to match Python’s 0-based dd[0, 1:ntcell + 1]
    
    for m in 1:iorder-1  # Adjusted from 2:iorder since m starts at 1 in Python
        for j in 1:ntcell-1  # Adjusted from 2:ntcell-1 to match Python’s 1:ntcell
    ...
    for i in 0:nx
        il[i] = i
        ir[i] = i + 1
        for m in 1:iorder-1  # Adjusted from 2:iorder
