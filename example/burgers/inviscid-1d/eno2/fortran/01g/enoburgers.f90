...
subroutine reconstruction(q)
    ...
    do j=ist-1, ied
        ...
        up1_2p(jj)=0
        do m=0,iorder-1 
            up1_2m(jj)=up1_2m(jj)+q(k1+m)*coef(l1,m)
            up1_2p(jj)=up1_2p(jj)+q(k2+m)*coef(l2,m)
        enddo  
    enddo        
...
subroutine engquist_osher_flux(up1_2m,up1_2p,flux)
    ...
    do i = ist-1, ied
        ...
        if ( up1_2m(ii) >= 0 ) then
            if ( up1_2p(ii) >= 0 )  then
                flux(ii) = 0.5 * up1_2m(ii) * up1_2m(ii)
            else 
                flux(ii) = 0.5 * ( up1_2m(ii) * up1_2m(ii) + up1_2p(ii) * up1_2p(ii) )
            endif    
        else
            if ( up1_2p(ii) >= 0 )  then
                flux(ii) = 0
            else
                flux(ii) = 0.5 * up1_2p(ii) * up1_2p(ii)
            endif
        endif    
    enddo            
