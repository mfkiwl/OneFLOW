...
def create_matrix(r,s,k,lr):
    ...
    for ii in range(-r,s+1):
        #print(f'{ii=},{r=},{s=},{r+s=}')
        x = compute_diff_coef(ii+ishift,k)
        #print(f"{x=}")
        # 动态地生成一些一维数组并添加到列表中
...
def calc_eno_coef(r,s,k,lr):
    matrix = create_matrix(r,s,k,lr)
    ...
    m1 = compute_coef(xi,k).reshape(1, -1)
