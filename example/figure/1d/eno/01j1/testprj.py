...
def plot_label(x, xcc, yref, ishift):
    ...
    str1 = genstrXNhalf('ied', i1)
    str2 = genstrXNhalf('ied', i2)
    ...
    str1 = genstrNp('ist',i1+ast)
    str2 = genstrNp('ist',i2+ast)
    ...
    str1 = genstrNp('ied',i1+ast)
    str2 = genstrNp('ied',i2+ast)
...
def plot_ghost_label_left(xg, xgcc, yref, ishift):
    ...
    for igc in range(ngc):
        ii = iighost+( 0+ishift-igc ) - 1
        ii = 2 * ii + 1
        #ii = -1 + 2 * ishift - 2 * igc
        strii = genstrXNhalf('', ii)
        plt.text(xg[igc+1], yb, strii, fontsize=12, ha='center')

    for igc in range(ngc):
        ii = iighost+( 0+ishift-igc )
        strii = genstrNp('',ii)
...
def plot_ghost_label_right(xg, xgcc, yref, ishift):
    ...
    for igc in range(ngc):
        ii = 3 + 2 * ishift + 2 * (igc+ast)
        stri = genstrXNhalf('ied', ii)
        plt.text(xg[igc+1], yb, stri, fontsize=12, ha='center')
    
    for igc in range(ngc):
        ii = 1+ishift+igc
        strii = genstrNp('ied',ii+ast)
        plt.text(xgcc[igc], ybc, strii, fontsize=12, ha='center')    
