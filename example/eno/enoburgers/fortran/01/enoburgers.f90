!subroutine reconstruction(u,nx,up1_2m,up1_2p,dd,il,ir,coef,iorder,ighost,dx,res)
!    implicit none
...
subroutine reconstruction(u,nx,up1_2m,up1_2p,dd,il,ir,coef &
                 ,iorder,ighost)
    implicit none
    real(8) :: u(-ighost:nx+ighost)
    real(8) :: up1_2m(0:nx),up1_2p(0:nx)            
...
!   2nd-order coefficients       
    !coef = reshape([1.5,-0.5,0.5,0.5,-0.5,1.5], [iorder+1, iorder], order = [1, 2])
    icount = 1
    do i = -1, iorder-1
        do j = 0, iorder-1
...
!   Time stepping
    t = 0                      
    do while ( t < supt ) 
        !call reconstruction(pu,nx,up1_2m,up1_2p,dd,il,ir,coef,iorder,ighost,dx,res)
        ...
        !call reconstruction(u1,nx,up1_2m,up1_2p,dd,il,ir,coef,iorder,ighost,dx,res)
        call reconstruction(u1,nx,up1_2m,up1_2p,dd,il,ir,coef,iorder,ighost)
        call getflux(up1_2m,up1_2p,flux,nx)
        do i = 1, nx
            ress = - ( flux(i) - flux(i-1) ) / dx
            u2(i) = 3.0/4.0 * pu(i) + 1.0/4.0 * u1(i) + 1.0/4.0 * dt * ress
        enddo
        
        call boundary( u2, nx, ighost )
        
        !call reconstruction(u2,nx,up1_2m,up1_2p,dd,il,ir,coef,iorder,ighost,dx,res)
        ...
        do i = 1, nx                                
            ...
            t2=2.0/3
            ress = - ( flux(i) - flux(i-1) ) / dx
            su(i) = t1 * pu(i) + t2 * u2(i) + t2 * dt * ress
        enddo
...
!   Output results to Tecplot file
    ...
    write(1, *) 'VARIABLES = "x", "Numerical", "Initial"'
    write(1, *) 'ZONE T="Solution", I=', nx + 1, ', F=POINT'
    do i = 0, nx
        write(1, *) x(i), pu(i), u0(i)
    end do
    close(1)
