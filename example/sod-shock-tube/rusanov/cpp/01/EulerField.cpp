...
void EulerField::Init( Grid * grid )
{
    ...
    int nm = 1;
    this->utest.Allocate( nm, ist, ied );
...
void EulerField::InviscidResidual( Vec1d & u, Vec1d & res )
{
    this->InviscidConservative( u, res );
}

void EulerField::WaveSpeed( Vec1d & u, Vec1d & ps )
{
    for ( int i = 0; i <= nx; ++ i )
    {
...
void EulerField::LaxFriedrichs( Vec1d & u, Vec1d & res )
{
    Vec1d fL;
    fL.Allocate( 0, nx, 0 );
...
void EulerField::burgers_fluxes( int ist, int ied, Vec1d & u, Vec1d & f )
{
    for ( int i = ist; i <= ied; ++ i )
    {
        f[ i ] = 0.5 * u[ i ] * u[ i ];
...
void EulerField::Rusanov( Vec1d & u, Vec1d & res )
{
    Vec1d uL, uR;
    uL.Allocate( 0, nx, 0 );
    uR.Allocate( 0, nx, 0 );
...
void EulerField::InviscidConservative( Vec1d & u, Vec1d & res )
{
...
void EulerField::ViscousResidual( Vec1d & u, Vec1d & res )
{
    ;
}

void EulerField::Rhs( Vec1d & u, Vec1d & res )
{
    res = 0;
...
void EulerField::DumpField( Vec1d & x, Vec1d & u )
{
