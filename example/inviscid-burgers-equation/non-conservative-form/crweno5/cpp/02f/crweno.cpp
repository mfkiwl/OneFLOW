...
//-----------------------------------------------------------------------------
void rhs_crweno( int ni, double dx, Vec1d & u, Vec1d & r )
{
    std::vector<double> uL( ni + 1, 0 );
    std::vector<double> uR( ni + 1, 0 );
...
void runge_kutta_stage1( RhsPtr rhs, int ni, Vec1d & u, Vec1d & un, Vec1d & res, double dx, double dt )
{
    rhs( ni, dx, u, res );
    for ( int i = 0; i < ni; ++ i )
    {
...
void runge_kutta_stage2( RhsPtr rhs, int ni, Vec1d & u, Vec1d & un, Vec1d & res, double dx, double dt )
{
    rhs( ni, dx, u, res );
...
void runge_kutta_stage3( RhsPtr rhs, int ni, Vec1d & u, Vec1d & un, Vec1d & res, double dx, double dt )
{
    rhs( ni, dx, u, res );
...
void numerical( RhsPtr rhs )
{
    ...
    {
        runge_kutta_stage1( rhs, ni, u, un, res, dx, dt );
        boundary( ni, u );
        runge_kutta_stage2( rhs, ni, u, un, res, dx, dt );
        boundary( ni, u );
        runge_kutta_stage3( rhs, ni, u, un, res, dx, dt );
        boundary( ni, u );
        un = u;
