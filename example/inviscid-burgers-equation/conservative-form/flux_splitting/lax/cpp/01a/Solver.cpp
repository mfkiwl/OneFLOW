...
void Solver::Init()
{
    ...
    if ( Global::scheme.inviscid == to_int( BasicScheme::WENO ) ||
         Global::scheme.inviscid == to_int( BasicScheme::CRWENO ))
    {
        Global::nghost = 3;
    }
    else
...
void Solver::InitTopo()
{
    ...
    for ( int iZone = 0; iZone < LocalZone::nZones; ++ iZone )
    {
        ...
        for ( int ibc1to1 = 0; ibc1to1 < nbc1to1s; ++ ibc1to1 )
        {
            ...
            interface->CalcInterface( &transform, region.start, region.end, donor_zoneid, Global::nghost );
        }
        int nInterfaces = interface->zoneList.size();
        int ngsize = Global::nghost + 1;
