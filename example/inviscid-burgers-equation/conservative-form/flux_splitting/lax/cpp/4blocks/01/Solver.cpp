...
{
    Parallel::Init();
    //std::ifstream f("../cfd.json");
    std::ifstream f("../burgers.json");
    //std::ifstream f("../heat.json");
    //std::ifstream f("../heaticp.json");
        ...
        for ( int iSend = 0; iSend < nsend_zones; ++ iSend )
        {
            ...
            for ( int iFace = 0; iFace < nInterFaces; ++ iFace )
            {
                ...
                for ( int ig = Global::ifinite_volume; ig <= Global::nghost; ++ ig )
                {
                    ...
                    donordata_for_send[ data_pos + iig ] = field->u[ id_cell ];
                }
            }
        }
    }
        ...
        for ( int iNei = 0; iNei < ndonor_zones; ++ iNei )
        {
            ...
            if ( Parallel::pid == recv_pid )
            {
                ...
                for ( int i = 0; i < neighbor_donorfaces.size(); ++ i )
                {
                    ...
                    int data_pos = local_faceid * ngsize;
                    int donor_data_pos = i * ngsize;

                    for ( int ig = Global::ifinite_volume; ig <= Global::nghost; ++ ig )
                    {
                        int iig = ig - Global::ifinite_volume;
                        double donor_value = donordata[ donor_data_pos + iig ];
                        donor_interface->data_recv[ data_pos + iig ] = donor_value;
                    }
                }
            }
        }
...
void Solver::DownloadInterfaceField()
{
    for ( int iZone = 0; iZone < LocalZone::nZones; ++ iZone )
    {
        ...
        for ( int iFace = 0; iFace < nInterFaces; ++ iFace )
        {
            ...
            {
                int iig = ig - Global::ifinite_volume;
                int ig_cell = interface->ijk_ghosts[ ijkpos + iig ] - 1;
                double donor_value = interface->data_recv[ data_pos + iig ];
                if ( ig == 0 )
                {
                    double valueb = 0.5 * ( field->u[ ig_cell ] + donor_value );
                    field->u[ ig_cell ] = valueb;
                }
                else
                {
